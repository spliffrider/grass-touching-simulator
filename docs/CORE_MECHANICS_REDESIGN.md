...
- First-run instruction density is presentation-aware: when the player/advisor side panels are visible, they replace the center Sensi callout and the bottom prompt stays visually quiet until the first touch. Very small layouts without the advisor retain the center callout. The objective, advisor, and `TOUCH GRASS` tile cue no longer repeat five versions of the same instruction.
...
- `?alpha` is the friend-facing public alpha route. It loads the redesign without developer controls, sets an Ancient Grass Alpha Test browser title, and shows a small dated `ALPHA TEST` build mark on the active-run HUD. `?redesign&playtest` remains the accelerated internal route.
