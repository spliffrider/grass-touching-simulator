...
13. The field expands through a tutorial/Guru quest, likely from 1x1 toward 2x2.
14. The first Scourge stage begins or is hinted.
15. The player eventually loses unless they play unusually well.
16. On dormancy, the game shows effective healing converted into permanent Grass Touches.
17. The permanent skill tree opens.
18. The player buys their first meta upgrade.
19. The next run starts stronger.
...
- The field now starts as one Ancient Grass/root tile, expands to 2x2 after the first real healing objective, expands to 3x3 after the Run Touch objective, and opens to 5x5 after the first wound triage objective.
...
- Dew Pulse now has first-pass discoverability cues: when it becomes usable, the button pulses, the run feed logs readiness, the bottom prompt calls out the spend, and Sensi briefly claims the advisor panel with a "buy time" line. Browser debug state reports `promptText`, `objectiveText`, and `lastDewPulseReadyAt` so smoke checks can verify the cue.
- The main run HUD now gives run-tool buttons their own lower row so Dew Pulse and Root Salve no longer collide with Run Touch text.
...
- The redesign route now uses the new Lucid music source as a browser-safe WAV asset (`public/assets/music/lucid-field-theme.wav`) generated from `docs/lucid.aif`, replacing the old mellow menu theme on this route.
- HP collapse now presents a real game-over/meta screen: the active run loop stops, the playfield is covered by a full-screen dormancy overlay, the run report calls out that the run is over, the reward line shows permanent GT gained, and the skill tree is the main available action before the next run.
...
- The meta screen now has a clearer first-pass skill tree layout: a run summary column, a Memory Skill Tree column, five functional memory nodes (`Soft Touch`, `Deeper Roots`, `Tiny Sprinkler`, `Scourge Sense`, and `Last Stand`), and an explicit `Begin Next Run` button. Random screen clicks no longer restart the run.
...
- After the first wound-triage lesson, ongoing wound pressure is presented as background pressure instead of repeating urgent tutorial copy over the dormancy objective.
- Browser debug state also reports Scourge presentation markers (`lastScourgeEvent`, `lastWoundPressureWarningAt`, `lastScourgePressureWaveAt`, `lastDormancyCollapseAt`, wound pressure progress, Scourge Sense ownership, Scourge Sense target, and Scourge Sense warning timestamps), `feedPanelVisible`, `runEnded`, `metaScreenVisible`, the next-run button, dormancy reward/report text, plus run-tool button bounds/state so smoke checks can verify threat beats, tool purchases, and high-level screen state even when canvas effects are brief.
...
Expansion is automatic, not manually purchased.

Use a hybrid trigger model:

- First expansion is Guru/tutorial quest driven.
- Main expansion is tied to survival and Scourge stages.
- Healing totals can act as readiness checks or bonus accelerators.
