...
export class AutomationIncomeSystem {
  ...
  update(delta: number, state: GameState, stats?: RuntimeStats): AutomationIncomeResult {
    ...
    // recordAutomationTouch already records one automated action internally, so
    // this single passive tick must not also call recordAutomationAction or it
    // would double-count automatedActions (which gates the "/25 actions" quest).
